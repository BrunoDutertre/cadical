...
class File {
  ...
  //BD: Windows makes a distinction between binary and text files
  //when we produce proofs in binary format, the proof file must
  //be opend in binary mode (otherwise '\n' is converted to '\r\n).
  ...
  // BD: added this
  static FILE * write_binary_file(Internal *, const char * path);
...
public:
  ...
  // And a variant for binary write
  static File * write_binary (Internal *, const char * path);
