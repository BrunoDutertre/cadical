...
// BD: this doesn't work on windows/mingw
#if 0
...
#else
// Try to get it to work on Windowa
// don't check /dev/null since that's not a Windows thing
bool File::writable (const char * path) {
  int res;
  if (!path) res = 1;
  else {
...
//BD:binary files
// on Windows, we open with mode "wb" to disable translation of '\n'
FILE * File::write_binary_file(Internal * internal, const char * path) {
  MSG ("opening binary file to write '%s'", path);
#ifdef _WIN32
...
File * File::write_binary (Internal * internal, const char * path) {
#ifdef _WIN32
  // on windows: we ignore xz, bz, etc.
  FILE *file = write_binary_file(internal, path);
  return file ? new File (internal, true, 1, file, path) : 0;
#else
  return write (internal, path);
#endif
}
